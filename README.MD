...
仔细思考就会发现，无论如何，爬虫最后一定会落实到 OneBookProcessor 身上，去对单个页面进行爬取，我们真正需要的是，
设计一种迭代策略，然后在迭代的最后一步调用 OneBookProcessor::doProcess(Page page)（"::"符号是java8的语法，这里指代一个方法）。

从我们的分析可以看到，迭代的策略一定需要持有一个 OneBookProcessor 实例，用于调用真正爬取单个页面的方法。
...
如果不是上级页面，则调用 ` processStrategy.doProcess(page) ` ，也就是 OneBookProcessor 。
...
在 IteratorProcessor 外面再加几层装饰，完成多层次的爬取工作，每一层只用专注于一层网页的爬取和信息收集工作，
如果这层的信息（往往是多个url）收集完毕，调用 ` page.addTargetRequests ` 方法之后，就可以交给更深层的 ProcessorStrategy 去完成工作了。

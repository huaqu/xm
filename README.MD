...
我们自定义了一个 CallablePipeline.java ，用来返回一个 Object 类型的结果。Pipeline不一定会完全将结果消费掉，
有些逻辑又不适合写到 Pipeline 中，这时我们会希望得到 Pipeline 中的结果，然后对结果单独处理。
webmagic觉得我们应该在 Pipeline 中完全消费掉结果（存到数据库，或者存到文件等等），
但是很多时候我们无法简单的在 Pipeline 中将结果消费掉，比如我们期待把这个结果返回到一个页面，
由一个工作人员确认爬取的信息无误，再将结果插入数据库。所以我们让 CallablePipeline 继承 Pipeline ，
并扩展了一个 getResult 方法。这样，我们可以随时从 Pipeline 中获取一个结果。
...
### 阶段总结：
我们将具体爬取数据的逻辑，与上层调用的逻辑，比较彻底的解耦了。SpiderService 持有了一个 DefaultSpider，
而DefaultSpider里面没有具体的实现，具体的实现通过 OneBookProcessor 实现。OneBookProcessor 专注于书籍页面的解析算法，
DefaultSpider 负责使用这个算法，并设置爬虫的各种性质。如果有了新的爬虫算法XXXProcessor，
